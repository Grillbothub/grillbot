...
`;
  ...
  var PaymentContainer = class extends HTMLElement {
    ...
    _updateAddToCartButton(variant) {
      ...
      console.log("addToCartButtonText",addToCartButtonText)
...
// Custom Review Slider Homepage
if ($('.custom-reviews-slider').length && $.fn.slick) {
  $('.custom-reviews-slider').slick({
    ...
    pauseOnHover: true,
    pauseOnFocus: true,
    prevArrow: '<div class="slick-prev"><img src="https://cdn.shopify.com/s/files/1/0337/8629/files/Vector_31.svg?v=1681299786"/></div>',
    nextArrow: '<div class="slick-next"><img src="https://cdn.shopify.com/s/files/1/0337/8629/files/Vector_30_1.svg?v=1681299786"/></div>',
    responsive: [
      {
        breakpoint: 991,
        settings: {
...
//         textureLoader.load( 
//             "https://cdn.shopify.com/s/files/1/0337/8629/files/pngwing_17.png?v=1683346574",
//             (texture) => {
...
// Pointer Dots JS
  $('.image-pointers-custom-wrapper span.pointer--dot').click(function(){
    $(this).siblings('.pointer-slide--wrapper').addClass('show-this-point');
    $(this).parents('.pointer-slide').siblings().find('.pointer-slide--wrapper').removeClass('show-this-point');
  });

  $('span.close-point').click(function(){
    $('.pointer-slide--wrapper').removeClass('show-this-point');
  })

// Product Thumbnails Slider
  // $('.custom-product-template--wrapper .product__thumbnail-list-inner').slick({
  //   slidesToShow: 3,
  //   slidesToScroll: 1,
  //   arrows: false,
  //   infinite: false,
  ...
  if ($('.custom-product-template--wrapper .product__thumbnail-list-inner').length && $.fn.owlCarousel) {
  $('.custom-product-template--wrapper .product__thumbnail-list-inner').owlCarousel({
		loop: false,
		autoplay: false,
        navigation: false,
  ...
  if ($(window).width() < 768 && $.fn.owlCarousel && $('.custom-reviews-section .reviews-slider').length) {
    $('.custom-reviews-section .reviews-slider').owlCarousel({
      loop: true,
      autoplay: true,
      navigation: false,
      autoHeight:true,
...
// Product Tabs Show Hide
  $('.product-feature-tabs li.tabs-slide').click(function(){
    var target = $(this).data("target");
    $('.product-feature-tabs .content__wrapper').removeClass('active-tab');
    $(target).addClass('active-tab');
    $(this).addClass('active-border');
    $(this).siblings().removeClass('active-border');
  })
$('.block-swatch__item').click(function(){
    ...
    var comparePrice = thisObj.attr('data-compare-price');

    if (!price) return;

    $('.product-form .price.price--large, .product-form .price.price--highlight.price--large').each(function () {
      var el = this;
      var hidden = el.querySelector('.visually-hidden');
      el.textContent = '';
      if (hidden) el.appendChild(hidden);
      el.appendChild(document.createTextNode(formatMoney(price).replace(/\s/g, '')));
    });

    if (comparePrice && Number(comparePrice) > Number(price)) {
      if ($('.product-form .price.price--compare').length) {
        $('.product-form .price.price--compare').each(function () {
          var el = this;
          var hidden = el.querySelector('.visually-hidden');
          el.textContent = '';
          if (hidden) el.appendChild(hidden);
          el.appendChild(document.createTextNode(formatMoney(comparePrice).replace(/\s/g, '')));
        });
      } else {
        $('.product-form .price.price--large, .product-form .price.price--highlight.price--large').first().after(
          '<span class="price price--compare"><span class="visually-hidden"></span>' + formatMoney(comparePrice).replace(/\s/g, '') + '</span>'
        );
      }

      var savings = formatMoney(Number(comparePrice) - Number(price)).replace(/\s/g, '').replace(/\.00$/, '').replace(/,00$/, '');
      $('.product-form [data-product-label-list]').html('<span class="label label--highlight">Save ' + savings + '</span>');
    } else {
      $('.product-form .price.price--compare').remove();
      $('.product-form [data-product-label-list]').empty();
    }
